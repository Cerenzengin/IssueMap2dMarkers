...
class SamplePinMarker extends Marker {
  ...
  // New properties for photos, description, and issue type
  ...
  constructor(markerData: MarkerData, title: string, description: string, image: HTMLImageElement, markerSet: SampleMarkerSet, scale: Range1dProps = { low: .75, high: 2.0 }, onMouseButtonCallback?: any) {
    ...
    // Additional properties for photos, descriptions, and issue type
    this.photos = markerData.photos || [];
    this.description = markerData.description || "";
    this.issueType = markerData.issueType || "";
  ...
  private showPopupMenu(cursorPoint: XAndY) {
    ...
    // Add entries for photos
    this.photos.forEach((photo, index) => {
      menuEntries.push({
        label: `Photo ${index + 1}`,
        onPicked: () => this.showPhoto(photo),
      });
    });

    // Add entries for description and issue type
    if (this.description) {
      menuEntries.push({ label: "Description", onPicked: () => this.showDescription() });
    }

    if (this.issueType) {
      menuEntries.push({ label: "Issue Type", onPicked: () => this.showIssueType() });
    }
  ...
  // Helper methods to handle different menu entries
  private showPhoto(photo: string) {
    // Implement the logic to display the photo
    console.log("Displaying photo:", photo);
  }

  private showDescription() {
    // Implement the logic to display the description
    console.log("Displaying description:", this.description);
  }

  private showIssueType() {
    // Implement the logic to display the issue type
    console.log("Displaying issue type:", this.issueType);
  }
}
  ...
  private getMenuItems(entries?: PopupMenuEntry[]): React.ReactNode[] {
    ...
    if (entries) {
      entries.forEach((entry: PopupMenuEntry, index: number) => {
        // Add conditions to exclude "Manual" entry based on your application logic
        if (!(entry.label === "Manual" && /* your condition here */)) {
          const item = this.getMenuItems(entry, index);
